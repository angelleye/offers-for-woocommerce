        ...
        if($post->ID)
        {
            ...
            if($_product != false) {
                ...
                /* Products Addon and Offers Plugin meta check starts */
                  ...
                  foreach ($offers_product_addon as $n=>$offers_product_addon_values) {
                    if (array_key_exists('group', $offers_product_addon_values)) {
                        ...
                        for ($j = 0; $j < count($key); $j++) {
                            if(isset($offers_product_addon[$key[$j]]['type'])){
                                $arry[$k]['type'] = $offers_product_addon[$key[$j]]['type'];
                            }
                            if ((count($p) % 3) != 0) {
                                if ($offers_product_addon[$key[$j]]['value'] != '') {
                                        ...
                                        $arry[$k]['options'][$p]['price'] = $offers_product_addon[$key[$j + 1]]['price'];
                                    if ($p < count($key)) {
                                        $p++;
                                    }
                                }
                ...
                $p = 0;
                $res = array();
                foreach ($newArray AS $key => $line) {
                    $exp = explode('[', $line['input_name']);
                    if (strpos($line['input_name'], $exp[0]) !== FALSE) {
                        if (array_search($exp[0], $res)) {
                            $kh = array_search($exp[0], $res);
                            $newArray[$key]['options'] = array_merge($newArray[$kh]['options'],$newArray[$key]['options']);
                            unset($newArray[$kh]);
                        } else {
                            $res[$key] = $exp[0];
                        }
