...
<br>
<?php if( !empty($offer_args['authorization']) ) { ?>
    <p style="text-align: center;"><a style="background-color: #008CBA;border: none;color: white;padding: 12px 20px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;margin: 4px 2px;cursor: pointer;" href="<?php echo $offer_args['capture_redirect']; ?>"><?php echo __('Accept Offer', 'offers-for-woocommerce'); ?></a></p>
<?php } else { ?>
    <p style="text-align: center;"><a style="background-color: #008CBA;border: none;color: white;padding: 12px 20px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;margin: 4px 2px;cursor: pointer;" href="<?php echo $offer_args['product_url']; ?><?php echo (strpos($offer_args['product_url'], '?')) ? '&' : '?'; ?><?php echo '__aewcoapi=1&woocommerce-offer-id=' . $offer_args['offer_id'] . '&woocommerce-offer-uid=' . $offer_args['offer_uid']; ?>"><?php echo apply_filters('aeofw_offer_pay_button_label', __('Click to Pay', 'offers-for-woocommerce'), 'countered', $offer_args); ?></a></p>
<?php } ?>
<?php if (isset($offer_args['final_offer']) && $offer_args['final_offer'] == '1') {
    ...
    <br>
    <p style="text-align: center;"><a style="background-color: #008CBA;border: none;color: white;padding: 12px 20px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;margin: 4px 2px;cursor: pointer;" href="<?php echo $offer_args['product_url']; ?><?php echo (strpos($offer_args['product_url'], '?')) ? '&' : '?'; ?><?php echo 'aewcobtn=1&offer-pid=' . $offer_args['offer_id'] . '&offer-uid=' . $offer_args['offer_uid']; ?>"><?php echo apply_filters('aeofw_offer_counter_button_label', __('Click to Counter', 'offers-for-woocommerce'), 'countered', $offer_args); ?></a></p>
<?php } ?>

<?php if (isset($offer_args['offer_expiration_date']) && $offer_args['offer_expiration_date']) {
    printf('<br><br><p style="margin: 0 0 16px;font-size: 16px;text-align: center;font-family: inherit; text-align:center" >' . __('Offer expires on:', 'offers-for-woocommerce') . ' %s', date("m-d-Y", strtotime($offer_args['offer_expiration_date'])) . '</p>');} ?>
