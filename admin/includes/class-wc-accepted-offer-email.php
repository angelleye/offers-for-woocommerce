...
class WC_Accepted_Offer_Email extends WC_Email {
    ...
    public function __construct() {
        ...
        $this->title = __('Accepted offer', 'offers-for-woocommerce');

        // this is the description in WooCommerce email settings
        $this->description = __('Accepted Offer Notification emails are sent when a customer offer is approved by the store admin', 'offers-for-woocommerce');

        // these are the default heading and subject lines that can be overridden using the settings
        $this->heading = __('Accepted Offer', 'offers-for-woocommerce');
        $this->subject = __('[{site_title}] Accepted Offer ({offer_number}) - {offer_date}', 'offers-for-woocommerce');

        // Set email template paths
        $this->template_html 	= 'woocommerce-offer-accepted.php';
        $this->template_plain 	= 'plain/woocommerce-offer-accepted.php';
     ...
     *
     * @param array $offer_args Get the offer email arguments.
     *
     * @since 0.1.0
     *
     * @return void
