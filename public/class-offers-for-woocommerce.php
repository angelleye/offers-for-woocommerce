...
class Angelleye_Offers_For_Woocommerce {
    ...
    private function __construct() {
	    ...
	    add_action('woocommerce_before_order_object_save', array( $this, 'wc_before_order_save'));

	    add_action('angelleye_paypal_for_woocommerce_product_level_payment_action', array($this,'ae_paypal_express_payment_action'), 99, 3);
	    add_filter('woocommerce_paypal_express_checkout_intent_payment_action', array($this,'ae_paypal_express_checkout_intent_payment_action'), 10, 2);
        add_action('woocommerce_checkout_order_created', array($this,'ae_wc_checkout_order_created'));
        add_filter('angelleye_woocommerce_express_checkout_do_express_checkout_payment_request_args', array($this,'ae_paypal_express_checkout_payment_request_args'),10, 4);
        add_filter('angelleye_woocommerce_express_checkout_set_express_checkout_request_args', array($this,'ae_paypal_express_checkout_request_args'),10, 4);
        add_filter('paypal_for_woocommerce_order_invoice_id', array($this,'ae_paypal_express_order_invoice_id'), 10, 2);
	        ...
	        if( !empty($order_id) && $order_id > 0 ) {
		        ...
		        if( !empty($payment_method) && $payment_method == 'paypal' &&  class_exists('Angelleye_Offers_For_Woocommerce_Paypal')) {
			        ...
			        if ( ! empty( $process_payment['result'] ) && $process_payment['result'] == 'success' ) {
				        $process_payment_redirect = !empty($process_payment['redirect']) ? $process_payment['redirect'] : '';
			        }
			        ...
			        $process_payment_status = true;
			        $process_payment_redirect = $order->get_checkout_payment_url().'&offer_authorization=true&offer_status=counter&order_id='.$order_id;

			        $process_payment   = array(
				        'result' => 'success',
				        'redirect' => $process_payment_redirect,
				        'payment_method' => 'paypal_express',
				        'is_counter' => true,
			        );
		        }

		        if ( ! empty( $process_payment_status ) ) {
			        echo json_encode( array( "statusmsg" => 'payment_authorization', 'redirect'  => $process_payment_redirect ));
			        exit;
		        }
	        }
        }
