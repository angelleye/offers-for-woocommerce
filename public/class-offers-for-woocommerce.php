...
class Angelleye_Offers_For_Woocommerce {
    ...
    private function __construct() {
        ...
        if (function_exists('wc')) {
            ...
            /**
             * This will display the data of Product addon if plugin is activated
             */
     ...
     * @return false|string
     */
    public function angelleye_ofw_recent_offers( $atts ) {

        $atts = shortcode_atts( array(
            'is_cart_offers' => false,
        ), $atts, 'aeofwc_recent_offers' );

        ob_start();

        try {

            if( $this->is_enable_make_offer_on_cart()  && !empty( $atts['is_cart_offers'] ) && ( 'true' === $atts['is_cart_offers'] || '1' === $atts['is_cart_offers'] ) ) {
     ...
     * @param array  $cart_item     Get the cart_item
     * @param string $cart_item_key Get the cart_item_key
     *
     * @since 0.1.0
     *
     * @return void
        ...
        if ($cart_item_key && is_cart()) {
            ...
            if (!empty($offers_product_addon)) {
                ...
                foreach ($offers_product_addon as $key => $offerProducts) {
                    ...
                    if (!empty($offerProducts['options']) && is_array($offerProducts['options'])) {
     ...
     *
     * @param string $redirect Get the redirect url.
     *
     * @since 0.1.0
     *
     * @return mixed|string
    ...
    public function ofw_login_redirect($redirect) {
        ...
        if (isset($_GET['backto']) && !empty($_GET['backto']) && $_GET['ref'] == 'make-offer') {
            ...
            if( !empty( $_GET['is_cart_offer'] ) ) {

                return add_query_arg( [
                    'aewcobtn' => '1',
                    'is_cart_offer' => esc_attr( $_GET['is_cart_offer'] ),
                ], esc_url_raw($_GET['backto']) );
            }
    ...
    public function angelleye_ofwc_offer_button_output($is_archive = false, $btn_position_class = '' ) {
        ...
        if ( $button_options_general && isset( $button_options_general['general_setting_enable_offers_only_logged_in_users'] ) && $button_options_general['general_setting_enable_offers_only_logged_in_users'] != '' ) {
            ...
            if ( isset( $button_options_general['general_setting_enable_offers_hide_untill_logged_in_users'] ) && $button_options_general['general_setting_enable_offers_hide_untill_logged_in_users'] != '' && !is_user_logged_in() ) {
                return null;
            ...
            if ( isset( $button_options_general['general_setting_allowed_roles'] ) && $button_options_general['general_setting_allowed_roles'] != '' && is_user_logged_in() ) {
                ...
                $user_data = !empty($current_user->ID) ? get_userdata($current_user->ID) : '';
        ...
        if ($custom_tab_options_offers['enabled'] == 'yes' && $is_display_offer_button && $is_instock && $custom_tab_options_offers['on_exit'] != 'yes') {
            ...
            $btn_output = '<div class=" ' . esc_attr($aeofwc_offer_button_wrap_class) . '">';
            ...
            if ($req_login) {
                ...
                if ($is_archive) {
                    $button = '<a href="' . esc_url( $permalink ) . '" id="offers-for-woocommerce-make-offer-button-id-' . esc_attr( $post->ID ) . '" class="product_type_simple wp-element-button offers-for-woocommerce-make-offer-button-catalog button alt  ' . esc_attr( $button_class ) . ' ' . esc_attr( $btn_position_class ) . '" style="' . $custom_styles_override . '">' . wp_kses_post( $button_title) . '</a>';
                } else {
                    $button = '<button type="button" id="offers-for-woocommerce-make-offer-button-id-' . esc_attr(  $post->ID ) . '" class="wp-element-button single_add_to_cart_button offers-for-woocommerce-make-offer-button-single-product ' . esc_attr( $button_class ) . ' ' . esc_attr( $lightbox_class ) . ' button alt ' . esc_attr(  $btn_position_class ) . '" style="' . $custom_styles_override . '">' . wp_kses_post(  $button_title ) . '</button>';
                }
            }
    ...
    public function angelleye_ofwc_before_add_to_cart_button() {
        ...
        echo '<div id="offers-for-woocommerce-add-to-cart-wrap" class="offers-for-woocommerce-add-to-cart-wrap ' . esc_attr($is_on_right) . '" data-ofwc-position="' . esc_attr($button_position) . '"><div class="aofwc-first-div ' . esc_attr($button_position) . ' ' . esc_attr($no_price_product_class) . '">';
    }
    ...
    public function angelleye_ofwc_woocommerce_after_single_product_summary() {
        ...
        echo '<div class="' . esc_attr($hidden_class) . '">';
     ...
     *
     * @param object $post Get the post object.
     *
     * @since 0.1.0
     *
     * @return void
    ...
    public function angelleye_ofwc_add_custom_woocommerce_product_tab($tabs) {
        ...
        /**
         * It was returning false that may breck filter function and Offers tabs are not visible on the front end to avoid such a things we need to return $tabs.
         *
        ...
        if (empty($offer_name)) {
            if (is_user_logged_in()) {
                ...
                if (!empty($current_user->user_firstname)) {
                    ...
                    /**
                     * Use WP/WC billing first/last name.
                     */
    ...
    public function enqueue_scripts() {
        ...
        if (is_object($post)) {
            ...
            $angelleye_post_data = array();
            if (isset($_GET['offer-pid']) && !empty($_GET['offer-pid'])) {
                $tmproduct_data = get_post_meta($_GET['offer-pid'], 'tmproduct_data', true);
	            $angelleye_post_data = isset($tmproduct_data['post_data']) ? $tmproduct_data['post_data'] : array();
            }
            ...
            if (wp_script_is('offers-for-woocommerce-plugin-script')) {
                wp_localize_script('offers-for-woocommerce-plugin-script', 'offers_for_woocommerce_js_params', apply_filters('offers_for_woocommerce_js_params', array( 'is_product_type_variable' => $is_product_type_variable,
                    ...
                    'angelleye_post_data' => $angelleye_post_data
                )));
            }
     ...
     * @param float   $product_price  Get the product price.
     * @param float   $offer_total    Get the offer total.
     * @param int $offer_quantity get the offer quantity.
     *
     * @since 0.1.0
     *
    ...
    public function new_offer_form_submit() {
        ...
        if (function_exists('THEMECOMPLETE_EPO_CART')) {
            ...
            if (!empty($arr_main_array) && is_array($arr_main_array)) {

                foreach ($arr_main_array as $key => $value) {

                    if (!empty($value['name']) && 'offer_product_id' === $value['name']) {
                        $product_id = $value['value'];
                    }

                    if (!empty($value['product_data']) && is_array($value['product_data'])) {

                        foreach ($value['product_data'] as $key => $value) {
                            $_POST[$key] = $value;
                            $product_post_data[$key] = $value;
                        }

                        $epos = THEMECOMPLETE_EPO_CART()->add_cart_item_data_helper(array(), $product_id, $_POST);
                        $epos['post_data'] = $product_post_data;
                    }
                }
            }
        }
        ...
        if (in_array('woocommerce-product-addons/woocommerce-product-addons.php', $active_plugins) || array_key_exists('woocommerce-product-addons/woocommerce-product-addons.php', $active_plugins)) {
            foreach ($arr_main_array as $key => $value) {
                if (array_key_exists('product_addon_array', $value )) {
                    $p = $position = 0;
        ...
        if (isset($post["offer_product_id"]) && $post["offer_product_id"] !== '') {
            ...
            $formData['_is_cart_offer'] = ( $this->is_enable_make_offer_on_cart() && !empty( $post['ofwc_hidden_is_cart_offer'] ) ) ? $post['ofwc_hidden_is_cart_offer'] : '';
            ...
            if ($is_counter_offer) {
                ...
                $parent_post = array(
                    ...
                    'post_modified_gmt' => gmdate("Y-m-d H:i:s", current_time('timestamp' )),
            ...
            if (function_exists('THEMECOMPLETE_EPO_CART')) {
                if (!empty($epos)) {
                    update_post_meta($offer_id, 'tmproduct_data', $epos);
                }
            }
     ...
     * @since   0.1.0
     *
     * @param array $offer get the offer.
     * @param array $offer_meta get the offer meta.
     *
     * @return bool
     */
    protected function add_offer_to_cart($offer = array(), $offer_meta = array()) {
        if (!is_admin()) {
            ...
            if (!$found) {
                // WC()->cart->empty_cart();
                $tmproduct_data = get_post_meta($offer->ID, 'tmproduct_data', true);
                if (isset($tmproduct_data['post_data']) && !empty($tmproduct_data['post_data'])) {
                    foreach ($tmproduct_data['post_data'] as $key => $value) {
                        $_POST[$key] = $value;
                    }
                }
     ...
     *
     * @param boolean $purchasable Check product is purchasable or not.
     * @param object $product Get the product object.
     *
     * @since 2.3.22
     *
     ...
     *
     * @param boolean $purchasable Check product is purchasable or not.
     * @param object $variable Get the variable object.
     *
     * @since 2.3.22
     *
     * @return bool|mixed|void
     ...
     *
     * @param boolean $hide Get the variation display status.
     * @param int $product_id Get product_id.
     *
     * @since 2.3.22
     *
     * @return bool|void
     ...
     *
     * @param string $msg Get the message.
     * @param int $pid Get the product_id.
     *
     * @since 2.3.22
     *
     * @return void
     ...
     * Sets qty and price on any offer items in cart
     *
     * @param object $cart_object Get the cart_object.
     *
     * @since 0.1.0
     */
     ...
     *
     * @param  array $item Get the item array.
     * @param  array $values Get the item values.
     * @param  string $key Get the item key.
     *
     * @since 2.3.22
     *
     * @return mixed
    ...
    public function ae_ofwc_woocommerce_before_checkout_process() {
        ...
        foreach ($woocommerce->cart->get_cart() as $cart_item) {
            ...
            if (isset($cart_item['woocommerce_offer_id'])) {
                ...
                /**
                 * Lookup Offer - Make sure valid 'accepted-offer' or 'countered-offer' status
                 */
                $offer = get_post($pid);
     ...
     * @param  array $data Get the comment data.
     *
     * @return bool
     *
     * @since  1.2.0
     ...
     *
     * @global object $wpdb get wpdb object.
     * @param int $offer_id Get the offer_id.
     * @param string $emails Get the emails.
     * @param boolean $is_approve Get the email approval status.
     *
     * @since   1.2.0
     *
     * @return void
     ...
     * @param  int $offer_id Get the offer_id.
     * @param  int $product_id Get the product_id.
     * @param  int $variant_id Get the variant_id.
     * @param  string $emails Get the Emails.
     *
     * @return boolean
     *
     ...
     * Get the percentage.
     *
     * @param  float $num_amount Get the num_amount
     * @param  float $num_total Get the num_total
     *
     ...
     * @since 1.2.0
     *
     * @param boolean $is_counter_offer Get the counter offer status.
     *
     * @return void
     */
     ...
     *
     * @param  boolean $is_counter_offer Get the counter offer status.
     * @param  array $post_data Get the $post_data.
     *
     * @return bool|void
     */
	...
	public function wc_before_thankyou( $order_id ) {
		...
		if( !empty($payment_authorization)) {
			...
			if( !empty($payment_method) && $payment_method == 'paypal' ){
				...
				$offer_phone = get_post_meta($offer_id, 'offer_phone', true);
				$offer_company_name = get_post_meta($offer_id, 'offer_company_name', true);
				$offer_email = get_post_meta($offer_id, 'offer_email', true);
				...
				$product_total = get_post_meta($offer_id, 'offer_amount', true);

				$product_shipping_cost = get_post_meta($offer_id, 'product_shipping_cost', true);

				$comments = get_comments( array( 'post_ID' => $offer_id ) );
			...
			} elseif ( !empty($payment_method) && $payment_method == 'paypal_express' ){
				...
				$accepted_offer_mail_send = get_post_meta($order_id, '_accepted_offer_mail_send', true);
				$new_counter_offer_mail_send = get_post_meta($order_id, '_new_counter_offer_mail_send', true);
				$new_offer_mail_send = get_post_meta($order_id, '_new_offer_mail_send', true);

				if( !empty($get_order_status) && $get_order_status == 'processing' && empty($accepted_offer_mail_send)) {
					$post_status = 'accepted-offer';
					$email_class = 'WC_Accepted_Offer_Email';
					$template_name = 'woocommerce-offer-accepted.php';
					$template_html_path = untrailingslashit(OFW_PLUGIN_URL) . '/admin/includes/emails/';
					update_post_meta( $order_id, '_accepted_offer_mail_send', 1 );
				} elseif( !empty($get_order_status) && $get_order_status == 'on-hold' && $is_counter_offer && empty($new_counter_offer_mail_send)) {
					$post_status = 'countered-offer';
					$email_class = 'WC_New_Counter_Offer_Email';
					$template_name = 'woocommerce-new-counter-offer.php';
					$template_html_path = untrailingslashit(OFW_PLUGIN_URL) . '/public/includes/emails/';
					update_post_meta( $order_id, '_new_counter_offer_mail_send', 1 );
				} elseif( !empty($get_order_status) && $get_order_status == 'on-hold' && !$is_counter_offer && empty($new_offer_mail_send)) {
					$post_status = 'publish';
					$email_class = 'WC_New_Offer_Email';
					$template_name = 'woocommerce-new-offer.php';
					$template_html_path = untrailingslashit(OFW_PLUGIN_URL) . '/public/includes/emails/';
					update_post_meta( $order_id, '_new_offer_mail_send', 1 );
				}
				...
				$product = ( $variant_id ) ? wc_get_product($variant_id) : wc_get_product($product_id);
				$is_offer_buyer_countered_status = ( $post_data->post_status == 'buyercountered-offer' ) ? true : false;
				$product_qty = ( $is_offer_buyer_countered_status ) ? get_post_meta($offer_id, 'offer_buyer_counter_quantity', true) : get_post_meta($offer_id, 'offer_quantity', true);
				$product_price_per = ( $is_offer_buyer_countered_status ) ? get_post_meta($offer_id, 'offer_buyer_counter_price_per', true) : get_post_meta($offer_id, 'offer_price_per', true);
				$product_shipping_cost = get_post_meta($offer_id, 'offer_shipping_cost', true);
				...
				if ($is_offer_buyer_countered_status) {
					update_post_meta($offer_id, 'offer_quantity', $product_qty);
					update_post_meta($offer_id, 'offer_price_per', $product_price_per);
					update_post_meta($offer_id, 'offer_shipping_cost', $product_shipping_cost);
					update_post_meta($offer_id, 'offer_amount', $product_total);
				}
				...
				if (isset($email_class) && !empty($email_class)) {
					...
					$new_email->recipient = $recipient;
					$new_email->plugin_slug = 'offers-for-woocommerce';
					$new_email->template_html_path = $template_html_path;
				...
				if( !empty($post_status)) {
					...
					$new_email->plugin_slug         = 'offers-for-woocommerce';
					$new_email->template_html       = 'woocommerce-offer-received.php';
					$new_email->template_html_path  = untrailingslashit( OFW_PLUGIN_URL ) . '/public/includes/emails/';
					$new_email->template_plain      = 'woocommerce-offer-received.php';
					$new_email->template_plain_path = untrailingslashit( OFW_PLUGIN_URL ) . '/public/includes/emails/plain/';
    ...
    public function ofw_capture_authorization_payment( $offer_id, $post_status, $is_admin = false ) {
        ...
        if( isset($payment_authorization) && !empty($payment_authorization)) {
	        ...
	        if( !class_exists('AngellEYE_Utility')) {
		        include_once PAYPAL_FOR_WOOCOMMERCE_DIR_PATH . '/angelleye-includes/angelleye-utility.php';
	        }

	        $AngellEYE_Utility = new AngellEYE_Utility( 'paypal-for-woocommerce',VERSION_PFW);
	        $AngellEYE_Utility->payment_method = $payment_method;
            ...
            if( $post_status == 'accepted-offer' ) {
                ...
                if(!empty($order_id) && $order_id > 0 ) {
                    ...
                    if( !empty($payment_method) && $payment_method =="paypal" && class_exists('WC_Gateway_Paypal')) {
	                    $WC_Gateway_Paypal->capture_payment($order_id);
                    } elseif ( !empty($payment_method) && $payment_method =="paypal_express" ) {
	                    $AngellEYE_Utility->angelleye_wc_paypal_express_docapture($order_id);
                    }
	            ...
	            if(!empty($order_id) && $order_id > 0 ) {
		            if ( !empty($payment_method) && $payment_method == "paypal" && ! class_exists( 'Angelleye_Offers_For_Woocommerce_Paypal' ) ) {
			            $ofw_paypal->do_void( $order_id );
		            } elseif ( !empty($payment_method) && $payment_method == "paypal_express" ) {
			            $AngellEYE_Utility->angelleye_wc_paypal_express_dovoid($order_id);
		            }
		            ...
		            if( !empty($order) ) {
			            ...
			            if (  !empty($payment_method) && $payment_method == "paypal" && !class_exists( 'Angelleye_Offers_For_Woocommerce_Paypal' ) ) {
				            ...
				            if ( ! empty( $process_payment['result'] ) && $process_payment['result'] == 'success' ) {
					            $authorization = true;
					            $authorization_redirect_url = !empty($process_payment['redirect']) ? $process_payment['redirect'] : '';
				            }
			            } elseif ( !empty($payment_method) && $payment_method == "paypal_express" ) {
				            ...
				            $get_checkout_payment_url = $order->get_checkout_payment_url().'&offer_authorization=true&offer_status=counter&order_id='.$order_id;

				            $process_payment   = array(
				               'result' => 'success',
                               'redirect' => $get_checkout_payment_url,
                        ...
                        if ( ! is_admin() ) {
                            if ( ! empty( $authorization ) ) {
                                echo json_encode( array(
                                    ...
                                    'redirect'  => $authorization_redirect_url
                                ) );
                                exit;
                            }
                        } else {
                            ...
                            if ( ! empty( $authorization ) ) {
                                $offer_args['authorization']          = $authorization;
                                $offer_args['authorization_redirect'] = $authorization_redirect_url;
                                if( !empty($payment_method) && $payment_method == "paypal" ) {
	                                $offer_args['capture_redirect']  = ! empty( $authorization_redirect_url ) ? str_replace( 'paymentaction=authorization', 'paymentaction=sale', $authorization_redirect_url ) : '';
                                } elseif( !empty($payment_method) && $payment_method == "paypal_express" ) {
	                                $offer_args['capture_redirect'] = $authorization_redirect_url.'&payment_action=Sale';
                                }
                            }
	...
	public function wc_before_order_save( $order ) {
		...
		if( !empty($payment_authorization) && !empty($payment_status) && strtolower($payment_status) == 'voided') {
			exit();
		}
	}
	...
	public function ae_paypal_express_payment_action( $gateways, $request = null, $order_id = null ) {
		...
		if( !empty($woocommerce->cart->cart_contents) ) {
			foreach ( $woocommerce->cart->cart_contents as $cart_item_key => $cart_item ) {
				if( isset($cart_item['payment_authorization']) && !empty($cart_item['payment_authorization']) ) {
					$payment_authorization = true;
				}
			}
		}

		if(!empty($_GET['pay_for_order']) && $_GET['pay_for_order'] == true && !empty($_GET['offer_authorization']) && $_GET['offer_authorization'] == true && !empty($_GET['offer_status'] == "counter") ) {
			$payment_authorization = true;
		}

		if( !empty($_GET['order_id']) && $_GET['order_id'] > 0 ) {
			$is_payment_authorization = get_post_meta( $_GET['order_id'], '_payment_authorization', true );
			$is_counter_offer = get_post_meta( $_GET['order_id'], '_is_counter_offer', true );
			if(!empty($is_payment_authorization) && !empty($is_counter_offer)) {
				$payment_authorization = true;
			}
		}

		if( !empty($_GET['payment_action']) && $_GET['payment_action'] == 'Sale' ) {
			$payment_authorization = false;
			$gateway_setting->settings['payment_action'] = 'Sale';
			$gateway_setting->payment_action = 'Sale';
		}

		if(!empty($payment_authorization)) {
			$gateway_setting->settings['payment_action'] = 'Authorization';
	...
	public function ae_paypal_express_checkout_intent_payment_action( $sdk_intend, $gateways ) {
		...
		if(!empty($_GET['pay_for_order']) && $_GET['pay_for_order'] == true && !empty($_GET['offer_authorization']) && $_GET['offer_authorization'] == true && !empty($_GET['offer_status']) && $_GET['offer_status']  == "counter") {
			$payment_authorization = true;
			$sdk_intend = 'capture';
		}

		if( !empty($_GET['pay_for_order']) && $_GET['pay_for_order'] && !empty($_GET['payment_action']) && $_GET['payment_action'] == 'Sale' ) {
			$payment_authorization = false;
	...
	public function ae_paypal_express_checkout_payment_request_args( $paypal_request, $payment_gateway, $PP_express, $confirm_order_id ) {
        ...
        if( !empty($confirm_order_id) &&  $confirm_order_id > 0 ) {
	        $is_payment_authorization = get_post_meta( $confirm_order_id, '_payment_authorization', true );
	        $is_counter_offer         = get_post_meta( $confirm_order_id, '_is_counter_offer', true );
	        if( !empty($is_payment_authorization) && !empty($is_counter_offer)) {
		        $invoice_id                              = ! empty( $paypal_request['Payments'][0]['invnum'] ) ? $paypal_request['Payments'][0]['invnum'] : '';
		        $invoice_id                              = $invoice_id . '-authorized-countered-' . rand( 1000, 9999 );
		        $paypal_request['Payments'][0]['invnum'] = $invoice_id;
		        update_post_meta($confirm_order_id, '_offer_order_invoice_id' ,$invoice_id);
	        }
        }
	...
	public function ae_paypal_express_checkout_request_args( $paypal_request, $payment_gateway, $PP_express, $confirm_order_id ) {
		...
		if( !empty($confirm_order_id) &&  $confirm_order_id > 0 ) {
			...
			if( !empty($is_payment_authorization) && !empty($is_counter_offer)) {
			    ...
			    if(!empty($_GET['pay_for_order']) && !empty($_GET['payment_action'])) {
			        $returnurl = !empty($paypal_request['SECFields']['returnurl']) ? $paypal_request['SECFields']['returnurl'] : '';
				    $paypal_request['SECFields']['returnurl'] = $returnurl.'&payment_action='.$_GET['payment_action'];
			    }
			}
		}
		return $paypal_request;
	...
	public function ae_paypal_express_order_invoice_id( $invoice_id, $order_id ) {
        ...
        if( !empty($order_id) && $order_id > 0 ) {
	        $is_payment_authorization = get_post_meta( $order_id, '_payment_authorization', true );
	        $is_counter_offer         = get_post_meta( $order_id, '_is_counter_offer', true );
	        $offer_invoice_id         = get_post_meta( $order_id, '_offer_order_invoice_id', true );
	        if( !empty($is_payment_authorization) && !empty($is_counter_offer) && !empty($offer_invoice_id)) {
	            $invoice_id = $offer_invoice_id;
	        }
        }
     ...
     *
     * @param  object $raw_custom get the raw_custom.
     *
     * @since 2.3.22
     *
     ...
     *
     * @param  string $key get the $key.
     * @param  string $value Get the $values.
     *
     * @since 2.3.22
     *
     * @return void
     ...
     *
     * @param  boolean $boolean Get the Coupon active status.
     *
     * @return false|mixed
     */
     ...
     *
     * @param  boolean $response Get the responses.
     *
     * @since 2.3.22
     *
     * @return string|void
     ...
     *
     * @param  array $methods get the all shipping methods.
     *
     * @since 2.3.22
     *
     * @return mixed
     ...
     *
     * @param  array $rates Get the all rates.
     * @param  object $package Get the package object.
     *
     * @since 2.3.22
     *
     * @return array|mixed
     ...
     *
     * @param  array $classes Get the body class array.
     *
     * @since 2.3.22
     *
     * @return mixed
     ...
     * @since  0.1.0
     *
     * @param  string $link Get the Product cart link.
     * @param  object $product Get the product object.
     *
     * @return mixed|string
     */
     ...
     *
     * @param  int $product_id Get the product_id.
     * @param  boolean $count_only Get only cart or not variable.
     *
     * @since 2.3.22
     *
     * @return int|WP_Query
     ...
     *
     * @param  object $wp_admin_bar Get the admin bar object.
     *
     * @since 2.3.22
     *
     * @return void
     ...
     *
     * @param  string $display_key Get the Display key.
     *
     * @since 2.3.22
     *
     ...
     *
     * @param  boolean $bool Get the offer status.
     * @param  object $offer Get the offer object.
     *
     * @since 2.3.22
     *
     * @return false|mixed
     ...
     *
     * @param string $post_status Get the post status.
     * @param int $post_id Get the post_id.
     *
     * @since 2.3.22
     *
     * @return mixed|string
     ...
     *
     * @param  int $product_quantity Get the product quantity.
     * @param  string $cart_item_key Get the cart item key.
     * @param  object $cart_item Get the cart item object.
     *
     * @since 2.3.22
     *
     * @return mixed|string
    ...
    public function ofw_add_offer_endpoint() {
        ...
        if( $this->is_enable_make_offer_on_cart() ) {
            add_rewrite_endpoint('cart-offers', EP_PAGES);
        }
    }
     ...
     *
     * @param  array $items get the account menu items.
     *
     * @since 2.3.22
     *
     * @return mixed
    ...
    public function ofw_woocommerce_account_menu_items($items) {
        ...
        if (!empty($items)) {
            ...
            if( $this->is_enable_make_offer_on_cart() ) {
                $items['cart-offers'] = __('Cart Offers', 'offers-for-woocommerce');
            }
        }
     ...
     *
     * @param  array $query_vars Get the query variables.
     *
     * @since 2.3.22
     *
     * @return mixed
    ...
    public function ofw_woocommerce_get_query_vars($query_vars) {
        ...
        if (!empty($query_vars)) {
            ...
            if( $this->is_enable_make_offer_on_cart() ) {
                $query_vars['cart-offers'] = 'cart-offers';
            }
        }
     ...
     *
     * @param  string $title Get the endpoint title.
     * @param  string $endpoint Get the endpoint.
     *
     * @since 2.3.22
     *
     * @return mixed|string|null
    ...
    public function ofw_woocommerce_endpoint_offers_title($title, $endpoint) {
        ...
        if ($endpoint === 'cart-offers' && $this->is_enable_make_offer_on_cart() ) {
            $title = __('Cart Offers', 'offers-for-woocommerce');
        }
     ...
     *
     * @param  boolean $boolean Get the discount status.
     *
     * @since 2.3.22
     *
     * @return false|mixed
     ...
     * @param  object $product Get the product object.
     * @param  object $variation Get the variation object.
     * @param  int $qty Get the quantity.
     * @param  string $thirdPartyData Get the third party date.
     *
     * @since 2.3.22
     *
     * @return float|mixed
    ...
    public function ofw_make_offer_button_on_cart(  $cart_item ) {
        ...
        if( $this->is_enable_make_offer_on_cart() ) {
            ...
            $post_id = !empty($variation_id) ? $variation_id : $product_id;
            ...
            $custom_tab_options_offers = array(
                'enabled' => get_post_meta( $product_id, 'offers_for_woocommerce_enabled', true),
            );
            ...
            if ($custom_tab_options_offers['enabled'] == 'yes' && $is_display_offer_button && $is_instock ) {
                ...
                if ($req_login) {
                    ...
                    $button = '<a href="' . esc_url( $redirect_url ) . '" id="offers-for-woocommerce-make-offer-button-id-' . esc_attr( $post_id ) . '" class="product_type_simple wp-element-button offers-for-woocommerce-make-offer-button-catalog button alt  ' . esc_attr( $button_class ) . ' ' . esc_attr( $btn_position_class ) . '" style="' . $custom_styles_override . '">' . wp_kses_post( $button_title) . '</a>';
                } else {
                    $button = '<a href="' . esc_url( $permalink ) . '" id="offers-for-woocommerce-make-offer-button-id-' . esc_attr( $post_id ) . '" class="product_type_simple wp-element-button offers-for-woocommerce-make-offer-button-catalog button alt  ' . esc_attr( $button_class ) . ' ' . esc_attr( $btn_position_class ) . '" style="' . $custom_styles_override . '">' . wp_kses_post( $button_title) . '</a>';
                }
            ...
            echo apply_filters('angelleye_offers_for_woocommerce_button_output', $btn_output);
        }
    }
